...
func (conn *conn) readLoop() {
	...
	defer func() {
		...
		if r := recover(); r != nil {
			if conn.err == nil {
				conn.err = r.(error)
				log.Println("readLoop panic:", conn.err)
			}
		}
		conn.Close()
	...
	for !conn.closed {
		...
		if remain <= conn.inChunkSize {
			...
			// Unfinish
			log.Printf("Unfinish message(remain: %d, chunksize: %d)\n", remain, conn.inChunkSize)
...
func (conn *conn) received(message *Message) {
	...
	if message.Type == AGGREGATE_MESSAGE_TYPE {
		...
		for message.Buf.Len() > 0 {
			...
			if err != nil {
				log.Println("conn::received() AGGREGATE_MESSAGE_TYPE read sub type err:", err)
				return
			}
